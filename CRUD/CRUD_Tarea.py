...
def crear_tarea(conn, id_usuario, titulo, descripcion=None):
    sql = cargar_sql_por_nombre("../DataBase/Consultas/ConsultasTarea.sql","crear_tarea")
    try:
        cursor = conn.cursor()
...
def obtener_tarea(conn, id_tarea):
    sql = cargar_sql_por_nombre("../DataBase/Consultas/ConsultasTarea.sql","obtener_tarea")
    try:
        cursor = conn.cursor()
...
def obtener_tareas_por_usuario(conn, id_usuario):
    sql = cargar_sql_por_nombre("../DataBase/Consultas/ConsultasTarea.sql","obtener_tareas_por_usuario")
    try:
        cursor = conn.cursor()
...
def eliminar_tarea(conn, id_tarea):
    sql = cargar_sql_por_nombre("../DataBase/Consultas/ConsultasTarea.sql","eliminar_tarea")
    try:
        cursor = conn.cursor()
        cursor.execute(sql, (id_tarea,))
        conn.commit()
